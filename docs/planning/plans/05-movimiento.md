...
| 2026-07-27 | 5.5-5.13 | **Los glifos, recalibrados contra pantalla real.** El feedback de uso fue que no se distinguían: `READY` se leía como «un recuadro sin animación», la `X` de `FAIL` «no bien iluminada» y `OK` como «una diagonal». **Ninguna era un fallo de animación.** El design system fijaba rejilla, opacidades y duraciones pero **nunca el tamaño**, y la implementación asumió una celda de carácter: 25 puntos en ≈7.8sp dejan cada punto por debajo de dos píxeles físicos a 420dpi. Se corrige con `Spacing.GLYPH_CELLS = 2` (entero de celdas, para no salirse de la retícula monoespaciada) y `DOT_RATIO` 0.22 → 0.34. Además: `OK` pasa de antidiagonal a **flecha a la derecha** —una raya inclinada no significa «hecho» y se confundía con el control del historial—, `READY` pasa de punto respirando a **cara con parpadeo**, el control del historial pasa de tres puntos a la diagonal completa con `Palette.HANDLE_IDLE`, y el glifo se centra verticalmente contra el renglón en vez de colgar de la caja de línea. `GlyphTest`, 11 casos: la gramática no tenía **ni un test** pese a ser aritmética pura. Verificado en emulador con capturas ampliadas. |
| 2026-07-27 | — | **El icono de la app pasa a ser el glifo `READY`.** Era el `READY` *anterior* —un punto central suelto— y se quedó desincronizado al convertirse READY en una cara. Ahora `ic_launcher_foreground.xml` dibuja la matriz 5×5 entera con la misma geometría que `Glyph.kt`: paso uniforme y radio = 0.34 del paso, los 20 apagados al 18% y los 5 encendidos al 100%. **Los apagados no son decoración**: sin ellos esto sería un icono figurativo, y con ellos es una matriz de puntos que resulta que dibuja una cara, que es la única forma en que la §4.8 lo tolera. El punto de esquina queda a 31.7dp del centro, dentro de la máscara circular de 33dp que es el peor recorte que aplica un lanzador. Verificado en el emulador — hizo falta **reiniciarlo**: la caché de iconos de Android tiene como clave paquete+versión y sobrevive a desinstalar. |
| 2026-07-30 | — | **El icono de la app deja de ser un glifo y pasa a ser el prompt.** El chevron y el cursor de bloque sobre el degradado de la §4.2, derivados del asset de 512 que también da el favicon y el apple-touch-icon (`public/favicon.svg`). **El criterio no cambia** —lo primero que se ve en el cajón tiene que ser lo primero que se ve al abrirla—, cambia la respuesta: la matriz de 25 puntos se lee a 13sp en su renglón, pero a 24dp entre otros cincuenta iconos vuelve a ser el problema de superficie que la §4.4 documentó a otra escala, y esta vez no hay `GLYPH_CELLS` que subir porque el tamaño lo fija el lanzador. Sigue sin haber nada figurativo: son dos caracteres de la retícula, dibujados y no compuestos con la mono porque un icono no puede depender de que la fuente exista. El fondo pasa de `@color/ink` plano a un vector con las cinco paradas del degradado. La tinta ocupa 54.1 × 33.1dp y su esquina queda a **31.7dp** del centro, el mismo margen que dejaba el icono anterior contra la máscara de 33dp: no se apura al límite porque el borde exacto queda a merced del antialiasing de cada lanzador. |
