...
| # | Tarea | Estado | Notas |
|---|-------|--------|-------|
| 0.0 | Toolchain local: JDK 17, Android SDK (+ `adb`), variables de entorno. Verificar con `./gradlew assembleDebug` | ⬜ Listo | **Bloqueante real**: hoy no hay `java`, `gradle` ni `adb` en el `PATH`. Todo el andamiaje está sin compilar |
| 0.1 | Proyecto Gradle: version catalog, `settings.gradle.kts`, módulo `app`, Compose habilitado, `assembleDebug` en verde | 🔄 En curso | Ficheros escritos y sin compilar. Falta: generar el `gradle-wrapper.jar` (`gradle wrapper --gradle-version 9.6.1`) y **confirmar que las versiones del catálogo existen y encajan** |
| 0.2 | Firma de depuración reproducible + `adb install` en el dispositivo real | 🔒 Bloqueado | Necesita 0.1. Probar pronto en el móvil de destino (riesgo de fabricante) |
